export class Ball {
  ...
  update = (p1_y: number, p2_y: number) => {
    ...
    if (this.x - this.ballRadius < this.w * 0.025) {
      if (
        this.y + this.ballRadius > p1_y &&
        this.y - this.ballRadius < p1_y + this.p1_height
      ) {
        this.dx *= -1;
    ...
    else if (this.x + this.ballRadius > this.w - this.w * 0.03) {
      if (
        this.y + this.ballRadius > p2_y &&
        this.y - this.ballRadius < p2_y + this.p2_height
      ) {
        this.dx *= -1;
  ...
  restart = () => {
    ...
    if ((this.dx > 0 && this.speed < 0) || (this.dx < 0 && this.speed > 0)) {
      dir = true;
    }
    this.x = this.w / 2;
    this.y = this.h / 2;
